...
The goal of this project is to create a program that would enable us to run infinitely dangerous programs safely, by using cryptographic protocols that provide perfect secrecy. The initial goal described in [my first post](https://www.lesswrong.com/posts/TK8ptSJGvAqj2HaRr/) was to implement Min Liang's symmetric Quantum Fully Homomorphic Encryption (QFHE) scheme from [this paper](https://arxiv.org/abs/1304.5087v4). However, in [my second post](https://www.lesswrong.com/posts/wuESDsLmDYtMgAFeE/), I proved that the scheme was not perfectly secret, and I fixed the mistake in the scheme by making a key update during the evaluations. However, this fix enables us to simulate only affine gates, called Clifford gates. Afterwards, I realized in my third post that Fully Homomorphic Encryption is not the way to go. Instead, in order to provide perfect secrecy, and in order to do so efficiently, we need Secure Multi-Party Computation (MPC). This is the solution to our problem.
