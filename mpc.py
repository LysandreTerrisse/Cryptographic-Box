...
# The shares for Alice, Bob, and Charlie correspond to the same polynom iff they are stored at the same index in their lists.
alice = []
bob = []
charlie = []
...
class Index:
    ...
    def __xor__(self, q):
        ...
        # Alice, Bob, and Charlie each add their two shares for p and q together. They obtain new shares for the polynom p + q
    ...
    def __and__(self, q):
        ...
        # But the shares correspond to a polynom of degree 2 rather than one
        
        # Alice therefore generates a polynom f_A(x) = a * x + b of degree 1
        ...
        # Bob also generates a polynom f_B(x) of degree 1
        ...
        # Charlie also generates a polynom f_C(x) of degree 1
        ...
        # They then all add their own shares together in order to obtain the shares of the polynom f = f_A + f_B + f_C
        ...
        # This value will be useful in order to hide the y-intercept of the polynom p + q
        # They add the polynom f and the polynom p + q together
        # That is, they each add their share of f with their share of p + q
        # let's call this new polynom "broadcast", since it will be broadcasted
        ...
        # Let h be our polynom. Alice has h(1), Bob has h(2), and Charlie has h(3)
        # h(1) = a * 1² + b * 1 + c = a + b + c
        ...
        # Now, Alice generates a polynom g_A(x) of degree 1 of same y-intercept than f_A(x)
        ...
        # Bob also generates a polynom g_B(x) of degree 1 of same y-intercept than f_B(x)
        ...
        # Charlie also generates a polynom g_C(x) of degree 1 of same y-intercept than f_C(x)
        ...
        # They then all add their own shares together in order to obtain the shares of the polynom g = g_A + g_B + g_C
        # Note that this polynom has the same y-intercept than f
        ...
        # Now, they compute the polynom v - g
        # Note that the y-intercept of this new polynom is f(0) + (p * q) - g(0) = p * q
...
def encrypt(x, base=2):
    ...
    # We create a random polynom whose y-intercept is the secret x
    a, b = getrandbits(2), x
...
def decrypt(index):
    ...
    # We have the shares of a polynom p(x) = ax + b of degree 1
